...
def preprocess_scene(scene_dir: str, scene_id: str, out_dir: Optional[str] = None) -> dict:
    """Preprocess a generated PNG scene using the pipeline's JSON contract."""
    ...
    filtered = SpeckleFilter(filter_type=FilterType.LEE, window_size=5).apply(raw)
    db_image = IntensityToDecibel().convert(filtered)
    low, high = np.percentile(db_image, [2.0, 98.0])
    if high <= low:
        high = low + 1e-6
    normalized = np.clip((db_image - low) / (high - low), 0.0, 1.0)
    ...
    with open(meta_path, "w", encoding="utf-8") as file:
        json.dump(out_meta, file, indent=2)
    return out_meta
